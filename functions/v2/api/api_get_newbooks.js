...
// 新刊一覧の取得
const getNewBooks = app.regionFunctions.https.onRequest(async (request, response) => {
    ...
    try {
        const newBooks = await db.getNewBooks(userId);
        let resBody = common.createResponseBody(common.R_SUCCESS);
        resBody["newBooks"] = newBooks;
        return response.send(JSON.stringify(resBody));     
    } catch (err) {
        logger.log(error.ERRMSG_API_GET_NEWBOOKS_ERROR, utils.Err2JSON(err));
        return response.send(JSON.stringify(common.createResponseBody(common.R_FAILURE, error.ERRMSG_API_GET_NEWBOOKS_ERROR)));
    }
});
